...
Provider admin APIs are called via `fetch` (LaunchDarkly / Unleash REST) — not the evaluation SDKs. Codemod preview/apply uses `ts-morph` in-process (diff-first; no silent writes). Pin resolved versions in `package.json`.
